...
function main()
    ...
    function ChangeState!(DirectionCommand::Char, Scalar::Int64, CurrentState::Vector{Float64}, CommandRegister::Dict{Char,Vector{Float64}})
        CommandRegister['F'] = [cosd(CurrentState[3]), sind(CurrentState[3]), 0]
    ...
    function Simulating(Commands::Vector{Tuple{Char,Int64}})
        ...
        for (command, scalar) in Commands
            ChangeState!(command, scalar, CurrentState, CommandRegister)
        end
        return CurrentState
    end
    ...
    CurrentState = Simulating(CommandsList)
